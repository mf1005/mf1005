...
当時、カラオケ機器（JOYSOUND/DAM/UGA等）の赤外線通信プロトコルを独学で解析。個人開発のWebサイト上の検索結果からiアプリ経由でカラオケ機へ選曲番曲を自動転送・予約するサービスを開発・公開していました。

当時のアプリ容量制限はわずか**10KB**。1バイトを削り出すために「クラスやメソッドをあえて作らず、すべてローカル変数で完遂させる」という、構造化やロジックの綺麗さを捨ててでもリソースに最適化させる極限のプログラミングを経験しました。
この **「1バイトの重み」を知る経験** や、**「バイナリを読み解く解析力」** 、**「制約の中で不可能を可能にする執念」** が、現在の私のエンジニアリングスタイルの根底に流れています。
