...
> - **「幽霊担当者」を用いたWBSで人員不足を数値化し、上層部の判断を強制的に促した**
> - **自部署の利益ではなくプロジェクト成功を優先し、納期直前のパニックを未然に回避**
> - **テスト不能とされたSaaS連携エラーを、Pythonによるスタブ即時構築で検証可能に改善**

<details>
<summary><b>詳細を表示</b></summary>

**役割：** 
開発担当（開発支援としてプロジェクトに参入。実態として開発工程も行いながらプロジェクトの不整合を正すため『嫌われ役』を遂行）
...
**行動と成果:**
1. **「肌感覚」を「物理的な数字」へ変換：**
WBS作成を自ら引き受け、全タスクを精査。現リソースでは到底埋まらないことを証明するため、あえて「要員Aさん～Eさん（存在しない幽霊担当者）」をアサインしなければ成立しない計画図を作成。
人員不足を言葉で訴えても「精神論」や「現場の努力不足」で返されるリスクがあったため、物理的に正しいWBSで不足を「見える化」し、上層部の議論を「どう人を確保するか」という具体策へ強制的にシフトさせた。

2. **技術による「工程の停滞」の即日解消（スタブ構築）：**
単体テスト工程において、「連携先SaaSが500エラーを返した際の挙動が確認できない」という課題に直面。「滅多に起きないからテスト不要では？」という妥協案が出そうになったが、ロジックの堅牢性を担保すべきと判断し、以下の「舗装」を即日実行。
- **仕様の解析：** SaaSのマニュアルを即座に確認し、エラー時のレスポンスボディ構造を特定。
- **スタブの構築：** **Pythonを用い、SaaSと同じ挙動（500/202/403等）を任意に返すスタブをその日のうちにGCP上に構築。**
- **結果：** 「テストができない」という言い訳を物理的に消滅させ、例外系ハンドリングの網羅的な検証を可能にした。

このアクションは、単なる独断専行ではなく、PLとの幾度とないインプットや、上長への事前相談・許可といった「組織としての順序」を踏んだ上で行っている。
プロジェクトメンバーとはランチを共にするような信頼関係があるからこそ、この「幽霊WBS」や「即席スタブ」が単なる批判ではなく、プロジェクトを救うための「共通の事実認識」として機能しました。

**ビジネス貢献:**
- **「手遅れ」になる前の軌道修正:** フェーズ移行直前という「まだ打ち手が間に合うタイミング」で上位層に現実を直視させ、人員補充や納期調整の議論を強制的に開始させた。
- **不確実性による停滞の打破:** 外部サービス（SaaS）の挙動待ちというボトルネックを技術で先行して「舗装」することで、開発チームが迷わずテストを完遂できる環境を整えた。
- **未然の炎上回避:** プロジェクトの「死」を予見し、物理的根拠をもってエスカレーションすることで、納期直前のパニックや現場の離脱という最悪のシナリオを回避。
- **事前の合意形成:** 現場の声を無視して一人で突き進むのではなく、**PLやメンバーが抱えていた『言い出せない不安』を数字や動くツールという共通言語に翻訳。**上位層へのエスカレーションも、現場の総意として合意形成を行った上でのアクションとなった。
