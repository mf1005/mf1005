...
> 文言: 「サーバー全体への影響が懸念されたため」  
> 文言: 「パフォーマンスの制限をかけさせていただきました」  
> 数値: Query_time: 10.048353 (1クエリに10秒かかっている絶望感)  
> 数値: Rows_examined: 670470 (フルスキャンに近い状態になっている)  
> SQL文: select max(Opponent_User_ID)... (group by で集計しており、これが重い原因だと一目でわかる)  
...
graph LR
 ...
 Cache["Cached Data<br>(JSON File)"]
 ...
 %% 1. 読み込み (Read)
 User == "1. Access" ==> App
 App == "2. Read File<br>(No DB Conn)" ==> Cache
...
事後マージ (追っかけ更新): Cronは「ベースとなるキャッシュ」を作り終えた後、「バッチ開始時刻 〜 現在」の間に更新されたレコードを再度DBから取得（Diff）。最後にこれをベースキャッシュにマージして上書き保存することで、データ欠損を防止。  
